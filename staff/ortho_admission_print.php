...
$query = "SELECT * FROM ortho_p_general JOIN ortho_p_init ON ortho_p_general.id = ortho_p_init.id JOIN patient_info ON ortho_p_general.id = patient_info.patient_id  JOIN patient_records ON ortho_p_general.id = patient_records.id JOIN ortho_p_insure ON ortho_p_general.id = ortho_p_insure.id WHERE ortho_p_general.id = $id;";
$res = $conn->query($query)->fetch_assoc();
//   print_r($res);
$query2 = "SELECT * FROM ortho_discharge WHERE id = $id;";
