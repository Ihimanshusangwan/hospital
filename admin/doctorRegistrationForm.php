...
<body>
  <div class="container mb-4">
    ...
    <h5 class="text-center mt-3">Doctor Registration Form</h5>
    <div class="row p-4 pt-4">
      <div class="col shadow-lg rounded m-2 p-4">
        <div class="text-info text-center ml-2">Doctor Details:</div>
        <?php
        require("connect.php");
       ...
       $mailErr = $passErr  = "";
          ...
          if (empty($mailErr) && empty($passErr)) {
            ...
            $sql = "INSERT INTO doctors (name, address, taluka, district, age, sex,dob, reg_date, mobile, email, password, type_of_visit)
            VALUES ('$name', '$address', '$taluka', '$district', '$age', '$sex', '$dob_date', '$reg_date', '$mobile', '$mail', '$pass', '$tov')";
            ...
            if ($conn->query($sql) === TRUE) {
              ...
              if ($_FILES['signature']['name'] != "") {
                ...
                $extension = end($ext);
                $newfile = "signatures/" . $id . "." . $extension;
                $tmpfile = $signature['tmp_name'];
                move_uploaded_file($tmpfile, $newfile);
                $query2 = "UPDATE doctors SET signature = '$newfile' WHERE id = '$id';";
                $conn->query($query2);
              }
        ...
        <form id="form" action="" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group m-2">
            <label for="tov">Type:</label>
            <select class="form-control" name="tov">
              <?php
